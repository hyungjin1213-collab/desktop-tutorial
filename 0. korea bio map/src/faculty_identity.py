...
def _openalex_affiliation(author: dict) -> str:
    ...
    return "; ".join(
        x.get("display_name", "") for x in institutions if isinstance(x, dict) and x.get("display_name")
    )
...
def _find_openalex_by_orcid(client: OpenAlexClient, orcid: str) -> dict | None:
    ...
    if not oid:
        return None
    for value in (oid, f"https://orcid.org/{oid}"):
        try:
            data = client._get("authors", {"filter": f"orcid:{value}", "per-page": 5})
        except Exception:
            continue
...
def match_faculty_identities() -> pd.DataFrame:
    ...
    if faculty.empty:
        ...
        out.to_csv(OUTPUT_DIR / "faculty_identity.csv", index=False, encoding="utf-8-sig")
        return out
    ...
    for _, person in faculty.iterrows():
        ...
        if author and orcid:
            ...
            if author:
                ...
                overlap = len(_tokens(university) & _tokens(oa_aff))
                if overlap >= 1:
                    confidence = "medium"
                    status = "probable"
        ...
        oa_id = normalize_openalex_id(author.get("id", "")) if author else ""
        oa_name = str(author.get("display_name", "")) if author else ""
        oa_aff = _openalex_affiliation(author) if author else ""
    ...
    out = pd.DataFrame(rows, columns=IDENTITY_COLUMNS)
...
def import_verified_faculty() -> int:
    ...
    existing_ids = list(professors.get("professor_id", []))
    nums = []
