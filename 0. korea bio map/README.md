...
- 왼쪽 아래 **은하 | 산학연병** 버튼: 산학연병을 누르면 대학(학)·병원(병)·연구소(연)·기업(산) 4개의
  원이 겹친 집합(벤 다이어그램)으로 바뀌고, 교수는 자기 소속 집합의 겹친 영역에 놓인다
  (예: 대학 + 대학병원 임상교수 → 학∩병).
- 소속은 최근 10년 논문의 본인 소속 기관 유형(OpenAlex: education/healthcare/facility·government/company)에서
  자동 계산한다. 논문의 20% 이상, 2편 이상에서 나온 유형만 인정 (`src/sectors.py`, `output/professor_sectors.csv`).
- 틀리면 `data/sector_overrides.csv`에 적는다: `P0001,학;산,바이오 스타트업 겸직`
- 필터 탭의 **산학연병** 칩으로도 거를 수 있다.
- 지금 DB는 대학 교수만 있어서 연·산 원은 겸직 교수만 들어간다. 연구소(KIST, 생명연 등)·기업 연구자를
  추가하면 채워진다.
